...
#' @examples none
#' @export
mergeFieldBookData <- function(fb, dj = NULL, scl = NULL, paperfb = NULL, trialDesigns = NULL, sqft = 45, testName = NULL, year = NULL, ...){
	# fb = fbL[[i]]; dj = scldj[["dj"]][[i]]; scl = scldj[["scl"]][[i]]; paperfb = pfbL[[i]]; trialDesigns = trials[grep(paste0("^", i), names(trials))]; sqft = 45; testName = i; year = yr; barley = isBarley
	if(is.null(testName)) {
	...
	if(!is.null(trialDesigns)){
		...
		if(trialClass == "trialDesign"){
			td <- lapply(trialDesigns, trialDesignToDataFrame)
			td <- do.call(rbind, td)
		} else {
